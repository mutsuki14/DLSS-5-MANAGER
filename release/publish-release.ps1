...
if ($info.commit -cne $env:GITHUB_SHA -or $info.tag -cne $meta.tag -or $info.repository -cne $repository) { throw 'Artifact provenance does not match this workflow.' }
...
if ($LASTEXITCODE -eq 0) {
    ...
    if ($existing.targetCommitish -cne $env:GITHUB_SHA) { throw 'An existing draft belongs to a different commit.' }
} else {
    & gh release create $meta.tag --repo $repository --target $env:GITHUB_SHA --title $meta.name --notes-file (Join-Path $PSScriptRoot 'notes.md') --draft --prerelease
...
$releaseJson = & gh api "repos/$repository/releases/tags/$($meta.tag)"
if ($LASTEXITCODE -ne 0) { throw 'Cannot verify uploaded assets.' }
$uploaded = $releaseJson | ConvertFrom-Json
